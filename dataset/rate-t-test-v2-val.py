...
abs_val_base_path = os.path.join(project_root, "dataset", "valid", "images")


# --- 後續的設定不變 ---
sampling_rate = 10
output_filepath = os.path.join(project_root, "dataset", "val-q.txt") 
...
with open(output_filepath, "w", encoding="utf-8") as f:
    ...
    try:
        img_list = os.listdir(abs_val_base_path)
    except FileNotFoundError:
        print(f"錯誤：找不到圖片目錄 {abs_val_base_path}")
    ...
    for index, fname in enumerate(img_list):
        if index % sampling_rate == 0:
            # 取得圖片的完整絕對路徑
            abs_file_path = os.path.join(abs_val_base_path, fname)
